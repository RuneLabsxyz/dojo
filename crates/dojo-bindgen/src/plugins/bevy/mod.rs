...
impl BevyPlugin {
    ...
    fn handle_struct(&self, token: &Token) -> TokenStream {
        ...
        match token {
            Token::Composite(c) => self.struct_handler.get(c),
    ...
    fn handle_enum(&self, token: &Token) -> TokenStream {
        match token {
            Token::Composite(c) => self.enum_handler.get(c),
    ...
    fn handle_model(&self, name: &str, model: &DojoModel) -> TokenStream {
        ...
        let structs = model.tokens.structs.iter().map(|e| self.handle_struct(e));

        let enums = model.tokens.enums.iter().map(|e| self.handle_enum(e));
